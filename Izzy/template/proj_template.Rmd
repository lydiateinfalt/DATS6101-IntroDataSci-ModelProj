...
```{r calc_models}
calc_models <- function(df, xdata, ydata, titleIn, xLabel, yLabel, colorBy) {
  #models 
  mod_lin <- nlsLM(ydata~lin_func(xdata, a1, b1), start=list(a1=1, b1=10))
  mod_quad <- nlsLM(ydata~quad_func(xdata, a1, b1, c1), start=list(a1=1, b1=1, c1=1))
  mod_cubic <- nlsLM(ydata~cubic_func(xdata, a1, b1, c1, d1), start=list(a1=1, b1=1, c1=1, d1=1))
  ...
  #plot
  data_plot <- ggplot(use_dat, aes(x = xLabel, y=yLabel, color=colorBy)) + 
    geom_point() +
    theme_minimal() +
    labs(title=titleIn, x=xLabel, y=yLabel) + 
    theme(panel.border = element_rect(colour = "black", fill=NA, size=1),
        axis.text.x = element_text(angle = 90, vjust = 0.5)) 
  data_plot + geom_line(aes(x=xdata, y=predict(mod_lin)), color="red") + 
    geom_line(aes(x=xdata, y=predict(mod_quad)), color="orange") +
...
swimming_age <- ggplot(swimming, mapping = aes(x = Year, y = mean.Age, group = Sex, color = Sex)) + geom_point(size=2) + geom_line() +
  labs(title="Average Age \nof Olympic Athletes \nin the Summer Games \nSeparated by Gender",x="year", y="mean age [years]") +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) 
# + geom_errorbar(aes(ymin=mean.Age-sd.Age, ymax=mean.Age+sd.Age), width=.2, position=position_dodge(0.05))
# swimming_age

swimming_weight <- ggplot(swimming, mapping = aes(x = Year, y = mean.Weight, group = Sex, color = Sex)) + geom_point(size=2) + geom_line() +
  labs(title="Average Weight \nof Olympic Athletes \nin the Summer Games \nSeparated by Gender",x="year", y="mean weight [kg]") +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) 
# + geom_errorbar(aes(ymin=mean.Weight-sd.Weight, ymax=mean.Weight+sd.Weight), width=.2, position=position_dodge(0.05))
# swimming_weight

swimming_height <- ggplot(swimming, mapping = aes(x = Year, y = mean.Height, group = Sex, color = Sex)) + geom_point(size=2) + geom_line() +
  labs(title="Average Height \nof Olympic Athletes \nin the Summer Games \nSeparated by Gender",x="year", y="mean height [kg]") +
  theme_minimal() +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1)) 
# + geom_errorbar(aes(ymin=mean.Height-sd.Height, ymax=mean.Height+sd.Height), width=.2, position=position_dodge(0.05))
# swimming_height
