...
olympics_summer_medals <- olympics %>% filter(Medal %in% Medals, Season == "Summer") %>%
group_by(Year) %>% tally(Medal.No.Yes)
...
ggplot(data=olympics_summer_medals, aes(x=Year, y=Medals)) +
  geom_bar(stat="identity", fill="steelblue")+ylab("Total Number of Medals") + theme_minimal()
...
new_data <- olympics %>% filter(Medal.No.Yes == "1") %>% group_by(Year) %>% add_tally(Medal.No.Yes, name="TMedals")
...
ggplot(new_data_subset, aes(Sex, fill = Medal.No.Yes)) + geom_density(alpha = 0.2)
...
olympic_logit <- glm(Medal.No.Yes ~ BMI + Age + Sex, data = new_data_subset, binomial(link = "logit"))
...
```{r AUC}
loadPkg("pROC") 
test_prob = predict(olympic_logit, newdata = new_data_subset, type = "response")
test_roc = roc(Medal.No.Yes~test_prob, data=new_data_subset, plot = TRUE, print.auc = TRUE)
...
```
```{r, results="asis"}
xkabledply(olympic_logit_confusion,"Confusion Matrix: Logit model 2, cutoff = 0.5")
```
